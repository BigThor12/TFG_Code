...
class SubwordTransformerWordEmbeddings(TransformerWordEmbeddings):
    ...
    def _add_embeddings_to_sentence(self, sentence: Sentence):
        ...
        if self.input_is_encoded:
            ...
            prep_tokens = [token.text.replace('<DOCSTART>', '1') for token in
                           sentence.tokens]  # convert <DOCSTART> to <pad>
            prep_tokens = ['1' if not token.strip() else token for token in
                           prep_tokens]  # convert empty tokens to <pad>
